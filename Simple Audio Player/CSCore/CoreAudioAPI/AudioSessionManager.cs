...
namespace CSCore.CoreAudioAPI
{
	...
	public class AudioSessionManager : ComObject
	{
		...
		/// <param name="sessionControl">The <see cref="AudioSessionControl"/> ot the specified <paramref name="audioSessionGuid"/>.</param>
		...
		public unsafe int GetAudioSessionControlNative(Guid audioSessionGuid, int streamFlags, out AudioSessionControl sessionControl)
		{
			...
			int result = InteropCalls.CallI(UnsafeBasePtr, &audioSessionGuid, streamFlags, &ptr, ((void**)(*(void**)UnsafeBasePtr))[3]);
			if (ptr != IntPtr.Zero)
		...
		public unsafe int GetSimpleAudioVolumeNative(Guid audioSessionGuid, NativeBool crossProcessSession, out SimpleAudioVolume audioVolume)
		{
			audioVolume = null;
			IntPtr ptr = IntPtr.Zero;
			int result = InteropCalls.CallI(UnsafeBasePtr, &audioSessionGuid, crossProcessSession, &ptr, ((void**)(*(void**)UnsafeBasePtr))[4]);
			if (ptr != IntPtr.Zero)
		...
		{
			SimpleAudioVolume v;
			CoreAudioAPIException.Try(GetSimpleAudioVolumeNative(audioSessionGuid, crossProcessSession, out v), "IAudioSessionManager", "GetSimpleAudioVolume");
			return v;
		}
	}
}
