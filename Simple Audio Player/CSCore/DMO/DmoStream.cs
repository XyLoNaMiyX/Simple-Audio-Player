...
namespace CSCore.DMO
{
    ...
    public abstract class DmoStream : IWaveSource
    {
        ...
        byte[] _inputBuffer;
        MediaBuffer _inputDataBuffer;
        WaveFormat _inputFormat;
        bool _isInitialized;
        MediaObject _mediaObject;
        DmoOutputDataBuffer _outputDataBuffer;

        int _outputDataBufferOffset;
        int _outputDataBufferOverflows;
        ...
        void DisposeAndResetObject<T>(ref T disposable) where T : class, IDisposable
        {
            if (disposable != null)
            {
                try
