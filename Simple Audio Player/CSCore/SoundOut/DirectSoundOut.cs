...
namespace CSCore.SoundOut
{
    ...
    public class DirectSoundOut : ISoundOut
    {
        ...
        DirectSound8 _directSound;

        DirectSoundNotify _directSoundNotify;
        bool _disposed;

        int _latency;
        volatile PlaybackState _playbackState;
        Thread _playbackThread;
        DirectSoundPrimaryBuffer _primaryBuffer;
        DirectSoundSecondaryBuffer _secondaryBuffer;
        IWaveSource _source;
        ...
        void InitializeInternal()
        {
            ...
            if (!_directSound.SupportsFormat(_source.WaveFormat))
            {
                ...
                var format16Bit = (WaveFormat) _source.WaveFormat.Clone();
                format16Bit.BitsPerSample = 16;
                var format8Bit = (WaveFormat)_source.WaveFormat.Clone();
        ...
        bool RefillBuffer(byte[] buffer, int sampleOffset, int bufferSize)
        {
            int read;
